#!/usr/bin/env python
# -*- coding: utf-8 -*-
#    chapter 13,    GAN,  generative adverasrial network
...
# 生成模型。目的是要生成一对 (z,L) 数据，其中z是噪声向量，L是 (1,28,28) 的图像空间。

def build_generator(latent_size):
	# We will map a pair of (z,L), where z is a latent vector and L is a label drawn from P_c, to iamge space (..., 1,28,28)
	cnn = Sequential()
	...
	# 10 classes in MNIST
	clss = Flatten()(Embedding(10, latent_size, embeddings_initializer='glorot_normal')(image_class))  # cls

	# hadamard product between z-space and a class conditional embedding
	h = layers.multiply([latent, clss])  #cls
...
def build_discriminator():
	...
	# first output (name=generation) is whether or not the discriminator thinks the image that is being shown is fake, and the second output (name=auxiliary) is the class that the discriminator thinks the image belongs to.
...
if __name__ == '__main__':
	...
	for epoch in range(epochs):
		...
		for index in range(num_batches):
			...
			# generate a batch of fake images, using the generated labels as a conditioner. We reshape the sampled labels to be (batch_size,1) so that we can feed them into the embedding layer as a length one sequence
			...
			# make new noise. we generate 2 * batch size here such that we have the generator optimize over an identical number of images as the discriminator
		...
		print('{0:<22s} | {1:4s} | {2:15} | {3:5s}'.format('component', *discriminator.metrics_names))
		print('-' * 65)
